...
**Canonical home: `weldebob/docket`, its default branch, `docs/graphr-system.md`.** The Docket thread commits every spec change there; Kith and the hub copy from it, byte for byte. No other branch or repo is a source. When copies disagree, that file wins; check the version line and md5 against it.
...
**Hazard, written down while it is cheap:** everything in §5 assumes **one person per account, and an account that person owns.** Many isolated accounts are fine; that's just many owners. **The shared person layer must break first** — before any of these ships:
- two accounts that can see the same obligations, people or edges (a shared board);
- any account that can read another's data (an admin, manager, coach or support view);
- an account owned or provisioned by an organisation rather than the person using it.

Docket's per-user boards (`8b_multiuser.sql`, isolated by RLS) are **the fine case**, not the hazard.
...
```
owner_profile                    -- one row per account
  user_id         uuid pk → auth.users.id
  display_name    text
  about_you       text null      -- assistant context; never sent with an invite
  timezone        text           -- required
  -- product-only columns are allowed here (e.g. Docket's plan, role, sections)

owner_contact_methods            -- same shape as contact_methods, plus:
  user_id         uuid → owner_profile.user_id
  share_personal  bool           -- travels with a personal invitation / card
  share_work      bool           -- travels with a work invitation / card
```

**Every row in every table is owned by exactly one account** (`user_id`), and row-level security isolates each account's data: `user_id = auth.uid()`. The owner is **one record per account**, not one per database. The sentinel rule is about *enumeration* — the owner never appears in `people` — not about row count.

**People are per-account and never shared across accounts.** Dana in Bob's account and Dana in Troy's are two unrelated records. There is no global person, no deduplication across accounts, and no lookup by email across accounts.

**Shared columns are identical in Kith and Docket; product-only columns may be added** to `owner_profile` and must be listed in that product's section. A product may keep a local table name during migration (Docket's `profiles`) only if a view named `owner_profile` exposes the shared columns.
...
| 3.5 | 2026-09-24 | **Canonical home is the Docket repo, default branch. Deliberate contact details stored, pasted text still sanitised; promise copy reworded. Docket: `captures` table for the catch tray, no direction column (follow-ups are your own obligations), `project` kept unshown, `⚠ HIGH` removed, owner sentinel applies. `owner_profile` is one row per account keyed on `user_id`; people never shared across accounts; hazard defined as shared boards, cross-account reads or org-owned accounts; `share_personal`/`share_work` replace `travels_with_invite`.** |
